                ...
                if (cx >= 0 && cx < _visibleLayerDescs[v]._width && cy >= 0 && cy < _visibleLayerDescs[v]._height) {
                    ...
                    if (_learn) {
                        ...
                        for (int c = 0; c < _visibleLayerDescs[v]._columnSize; c++) {
                            ...
                            float d = (c == inputIndexPrev ? _alpha : -_alpha) * _feedForwardWeights[v][hiddenCellIndexPrev][wi];
...
void Layer::columnBackward(int ci, int v, std::mt19937 &rng) {
    ...
    int visibleCellIndexInput = ci + _inputs[v][ci] * visibleWidth * visibleHeight;         
        ...
        for (int dcy = -backwardRadius; dcy <= backwardRadius; dcy++) {
            ...
            if (cx >= 0 && cx < _hiddenWidth && cy >= 0 && cy < _hiddenHeight) {
                ...
                if (!_feedBack.empty() && !_feedBackPrev.empty()) {
                    ...
                    columnActivationPrev += _feedBackWeights[v][visibleCellIndexInput][wiPrev];
    ...
    // Learn
    if (_learn) {
        float update = _beta * (_reward + _gamma * columnActivations[predIndex] - columnActivationPrev);
